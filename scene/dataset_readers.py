...
def readNerfSyntheticInfo2(path, white_background, depths, eval, extension=".png"):
...
def readColmapSceneInfo2(path, images, depths, eval, train_test_exp, llffhold=0):
    try:
    ...
    depth_params_file = os.path.join(path, "sparse/1", "depth_params.json")
    ## if depth_params_file isnt there AND depths file is here -> throw error
    depths_params = None
    if depths != "":
        try:
    ...
    # filter iphones
    cam_extrinsics = {k: v for k, v in cam_extrinsics.items() if "iphone" not in v.name}
    ...
    ply_path = os.path.join(path, "sparse/1/points3D.ply")
    bin_path = os.path.join(path, "sparse/1/points3D.bin")
    txt_path = os.path.join(path, "sparse/1/points3D.txt")
    if not os.path.exists(ply_path):
        print("Converting point3d.bin to .ply, will happen only the first time you open the scene.")
        try:
            xyz, rgb, _ = read_points3D_binary(bin_path)
...
sceneLoadTypeCallbacks = {
    ...
    "ScanNet": readNerfSyntheticInfo2,
    "Combined": readColmapSceneInfo2,
}